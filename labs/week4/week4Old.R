...
df2 <- data.frame(
  "year" = df$year[df$wdsp != "N/A" & df$wdsp != ""],
  "month" = df$month[df$wdsp != "N/A" & df$wdsp != ""],
  "meant" =  df$meant[df$wdsp != "N/A" & df$wdsp != ""],
  "maxtp" = df$maxtp[df$wdsp != "N/A" & df$wdsp != ""],
  "mintp" = df$mintp[df$wdsp != "N/A" & df$wdsp != ""]
)
...
# Filter columns with fewer unique values than the threshold
potential_columns <- names(unique_counts)[unique_counts < threshold]

# Print the potential columns
...
# df$sunshine <- cut(df$sun, breaks = breaks, labels = labels, include.lowest = TRUE)
# df$maxAirTemp <- cut(df$maxtp, breaks = breaks, labels = labels, include.lowest = TRUE)

#YearMon, year, month and temp and gather the avg meant monthly
# 50 -> Short, 100 -> Medium, 150 -> Long
# df2 <- df %>%
